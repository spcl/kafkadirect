...
object Defaults {
  ...
  val RDMAPort: Int = 0
  val RDMAHostName: String = new String("")
...
class KafkaConfig(val props: java.util.Map[_, _], doLog: Boolean, dynamicConfigOverride: Option[DynamicBrokerConfig])
  ...
  def RDMAListener: Option[EndPoint] = if(rdmaHostName.isEmpty) None else  Some(new EndPoint(rdmaHostName, rdmaPort, ListenerName.forSecurityProtocol(SecurityProtocol.RDMA), SecurityProtocol.RDMA))
