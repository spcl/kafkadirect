...
public class Fetcher<K, V> implements SubscriptionState.Listener, Closeable {
    ...
    public Fetcher(LogContext logContext,
        ...
        this(logContext, client, minBytes, maxBytes, maxWaitMs, fetchSize, maxPollRecords, checkCrcs,
                ...
                time, retryBackoffMs, requestTimeoutMs, isolationLevel, false,null, 0, 0, false, 0L, 0L);
    }
    ...
    public Fetcher(LogContext logContext,
                   ...
                   long retryBackoffMs,
                   long requestTimeoutMs,
                   IsolationLevel isolationLevel, boolean measure_latency, ConsumerRDMAClient rdmaClient,
        ...
        this.latencies_with_data = new LinkedList<>();
        this.latencies_no_data = new LinkedList<>();
    ...
    private static float[] percentiles(LinkedList<Float> latencies, double... percentiles) {
        int size = latencies.size();
        Collections.sort(latencies);
        float[] values = new float[percentiles.length];
    ...
    public void close() {
        ...
        if(!latencies_with_data.isEmpty() && latencies_with_data.size() > warmup){
            ...
            System.out.printf("Latency of fetch with data [Total %d measurements]: 50th = %f us, 95th = %f us , 99th = %f us, 99.9th = %f us %n",
                    latencies_with_data.size(),
                    percs[0],
                    percs[1],
                    percs[2],
                    percs[3]);
        }
        if(!latencies_no_data.isEmpty() && latencies_no_data.size() > warmup){
            for(int i =0 ; i<warmup; i++)   latencies_no_data.pollFirst();
            float[] percs = percentiles(this.latencies_no_data,  0.5, 0.95, 0.99, 0.999);
            System.out.printf("Latency of fetch without data [Total %d measurements]: 50th = %f us, 95th = %f us , 99th = %f us, 99.9th = %f us %n",
                    latencies_no_data.size(),
                    percs[0],
                    percs[1],
                    percs[2],
                    percs[3]);
